      ...
      <div class="mb-4 flex flex-col gap-4 sm:mb-6">
        ...
        <div v-if="activeTab === 'active'" class="tab-panel">
          ...
          <div v-if="apiKeysLoading" class="py-12 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400">{{ t('apiKeys.loading') }}</p>
          </div>
          ...
          <div v-else-if="apiKeys.length === 0" class="py-12 text-center">
            ...
            <p class="mt-2 text-sm text-gray-400">{{ t('apiKeys.noApiKeysHint') }}</p>
          </div>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm">
                      <div class="inline-flex items-center gap-1.5">
                        ...
                        >
                          ...
                          {{ t('apiKeys.activationDays', { days: key.activationDays || 30 }) }}
                        </span>
                        <!-- 已设置过期时间 -->
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm">
                      <div class="flex gap-1">
                        ...
                        >
                          ...
                          />
                          <span class="ml-1 hidden xl:inline">{{ t('common.model') }}</span>
                        </button>
                        <button
                          ...
                          :title="t('apiKeys.copyStatsLink')"
                          @click="copyApiStatsLink(key)"
                        >
                          <i class="fas fa-chart-bar" />
                          <span class="ml-1 hidden xl:inline">{{ t('common.stats') }}</span>
                        </button>
                        <button
                        ...
                        >
                          ...
                          <span class="ml-1 hidden xl:inline">{{ t('common.delete') }}</span>
                        </button>
                      </div>
                    </td>
                  </tr>
                        ...
                        >
                          <div class="mb-3 flex items-center justify-center gap-2">
                            ...
                            >
                              ...
                              <span class="text-xs">{{ t('apiKeys.resetFilter') }}</span>
                            </button>
                          </div>
                          <p class="text-xs text-gray-400">
                            {{ t('apiKeys.adjustTimeRange') }}
                          </p>
                        </div>
                        <div
                          v-else-if="
                            ...
                            <div class="mb-3 flex items-start justify-between">
                              <div class="flex-1">
                                ...
                                <span
                                  ...
                                  >{{ stat.requests }} {{ t('apiKeys.requests') }}</span
                                >
                              </div>
                            </div>
                ...
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                  <div class="mb-2 flex items-center justify-between">
                    ...
                    <button
                      ...
                      @click="showUsageDetails(key)"
                    >
                      <i class="fas fa-chart-line mr-1" />{{ t('apiKeys.viewDetails') }}
                    </button>
                  </div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-1">
                    <span
                      ...
                      "
                    >
                      {{ key.expiresAt ? formatDate(key.expiresAt) : t('apiKeys.neverExpire') }}
                    </span>
                    <button
        ...
        <div v-else-if="activeTab === 'deleted'" class="tab-panel">
          ...
          <div v-else-if="deletedApiKeys.length === 0" class="py-12 text-center">
            ...
            <p class="text-lg text-gray-500 dark:text-gray-400">{{ t('apiKeys.noDeletedKeys') }}</p>
            <p class="mt-2 text-sm text-gray-400">{{ t('apiKeys.noDeletedKeysHint') }}</p>
          </div>
          ...
          <div v-else>
            ...
            <div class="mb-4 flex justify-end">
              ...
              >
                ...
                {{ t('apiKeys.clearAllDeleted') }} ({{ deletedApiKeys.length }})
              </button>
            </div>
                  ...
                  <tr v-for="key in deletedApiKeys" :key="key.id" class="table-row">
                    ...
                    <td class="px-3 py-4">
                      <div class="text-sm">
                        ...
                        <div v-else class="text-xs text-gray-400">{{ t('apiKeys.neverUsed') }}</div>
                      </div>
                    </td>
                    <td class="px-3 py-4">
                      <div class="flex items-center gap-2">
                        ...
                        <button
                          ...
                          :title="t('apiKeys.permanentDelete')"
                          @click="permanentDeleteApiKey(key.id)"
                        >
                          <i class="fas fa-times mr-1" />
                          {{ t('apiKeys.permanentDelete') }}
                        </button>
                      </div>
                    </td>
                  </tr>
...
const timeRangeOptions = computed(() => [
  { value: 'today', label: t('apiKeys.timeRange.today'), icon: 'fa-clock' },
  { value: '7days', label: t('apiKeys.timeRange.week'), icon: 'fa-calendar-week' },
  { value: 'monthly', label: t('apiKeys.timeRange.month'), icon: 'fa-calendar' },
  { value: 'all', label: t('apiKeys.timeRange.all'), icon: 'fa-infinity' }
])
...
const loadApiKeys = async () => {
  ...
  try {
    ...
    showToast(t('apiKeys.loadFailed'), 'error')
  } finally {
...
const getBoundAccountName = (accountId) => {
  if (!accountId) return t('apiKeys.unknown')
  ...
  if (accountId.startsWith('group:')) {
    ...
    if (claudeGroup) {
      return `${t('apiKeys.belongsToGroup', { name: claudeGroup.name })}`
    }

    // 从Gemini分组中查找
    const geminiGroup = accounts.value.geminiGroups.find((g) => g.id === groupId)
    if (geminiGroup) {
      return `${t('apiKeys.belongsToGroup', { name: geminiGroup.name })}`
    }

    // 从OpenAI分组中查找
    const openaiGroup = accounts.value.openaiGroups.find((g) => g.id === groupId)
    if (openaiGroup) {
      return `${t('apiKeys.belongsToGroup', { name: openaiGroup.name })}`
    }

    // 如果找不到分组，返回分组ID的前8位
    return `${t('accounts.belongsToGroup', { name: groupId.substring(0, 8) })}`
  }
...
const getClaudeBindingInfo = (key) => {
  if (key.claudeAccountId) {
    ...
    if (!account) {
      return `⚠️ ${info} (${t('apiKeys.bindingWarning')})`
    }
    if (account.accountType === 'dedicated') {
      return `🔒 ${t('apiKeys.dedicated')}-${info}`
    }
    return info
  }
  if (key.claudeConsoleAccountId) {
    ...
    if (!account) {
      return `⚠️ ${t('apiKeys.consoleAccount')}${t('apiKeys.bindingWarning')}`
    }
...
const loadApiKeyModelStats = async (keyId, forceReload = false) => {
  ...
  try {
    ...
    showToast(t('apiKeys.loadingModelStats'), 'error')
...
const resetApiKeyDateFilter = (keyId) => {
  ...
  // 重新加载数据
  loadApiKeyModelStats(keyId, true)
  showToast(t('apiKeys.resetFilter'), 'info')
}
...
const openBatchEditModal = async () => {
  if (selectedApiKeys.value.length === 0) {
    showToast(t('apiKeys.selectKeysFirst'), 'warning')
    return
  }
...
const handleEditSuccess = () => {
  ...
  showToast(t('apiKeys.updateFailed'), 'success')
  loadApiKeys()
}
...
const handleRenewSuccess = () => {
  ...
  showToast(t('apiKeys.expiryUpdated'), 'success')
  loadApiKeys()
}
...
const toggleApiKeyStatus = async (key) => {
  ...
  if (key.isActive) {
    if (window.showConfirm) {
      confirmed = await window.showConfirm(
        t('apiKeys.confirmDisable').split('"')[0].trim(),
  ...
  try {
    ...
    if (data.success) {
      showToast(key.isActive ? t('apiKeys.keyDisabled') : t('apiKeys.keyActivated'), 'success')
      // 更新本地数据
      ...
      showToast(data.message || t('apiKeys.operationFailed'), 'error')
    }
  } catch (error) {
    showToast(t('apiKeys.operationFailed'), 'error')
  }
}
...
const deleteApiKey = async (keyId) => {
  ...
  if (window.showConfirm) {
    confirmed = await window.showConfirm(
      t('apiKeys.confirmDelete').split(' ')[0],
    ...
    // 降级方案
    confirmed = confirm(t('apiKeys.confirmDelete'))
  }
  ...
  try {
    ...
    if (data.success) {
      showToast(t('apiKeys.keyDeleted'), 'success')
      // 从选中列表中移除
      ...
      showToast(data.message || t('apiKeys.deleteFailed'), 'error')
    }
  } catch (error) {
    showToast(t('apiKeys.deleteFailed'), 'error')
  }
}
...
const restoreApiKey = async (keyId) => {
  ...
  if (window.showConfirm) {
    confirmed = await window.showConfirm(
      t('apiKeys.canRestore'),
      t('apiKeys.confirmRestore'),
      t('apiKeys.canRestore'),
      t('common.cancel')
    )
  } else {
    // 降级方案
    confirmed = confirm(t('apiKeys.confirmRestore'))
  }
  ...
  try {
    ...
    if (data.success) {
      showToast(t('apiKeys.keyRestored'), 'success')
      // 刷新已删除列表
      await loadDeletedApiKeys()
      // 同时刷新活跃列表
      await loadApiKeys()
    } else {
      showToast(data.error || t('apiKeys.restoreFailed'), 'error')
    }
  } catch (error) {
    showToast(error.response?.data?.error || t('apiKeys.restoreFailed'), 'error')
  }
}
...
const permanentDeleteApiKey = async (keyId) => {
  ...
  if (window.showConfirm) {
    confirmed = await window.showConfirm(
      t('apiKeys.permanentDelete'),
      t('apiKeys.confirmPermanentDelete'),
      t('apiKeys.permanentDelete'),
      t('common.cancel')
    )
  } else {
    // 降级方案
    confirmed = confirm(t('apiKeys.confirmPermanentDelete'))
  }
  ...
  try {
    ...
    if (data.success) {
      showToast(t('apiKeys.keyPermanentDeleted'), 'success')
      // 刷新已删除列表
      loadDeletedApiKeys()
    } else {
      showToast(data.error || t('apiKeys.deleteFailed'), 'error')
    }
  } catch (error) {
    showToast(error.response?.data?.error || t('apiKeys.deleteFailed'), 'error')
  }
}
...
const clearAllDeletedApiKeys = async () => {
  ...
  if (count === 0) {
    showToast(t('apiKeys.noDeletedKeys'), 'info')
    return
  }
  ...
  if (window.showConfirm) {
    confirmed = await window.showConfirm(
      t('apiKeys.clearAllDeleted'),
      t('apiKeys.confirmClearAll', { count }),
      t('apiKeys.clearAllDeleted'),
      t('common.cancel')
    )
  } else {
    // 降级方案
    confirmed = confirm(t('apiKeys.confirmClearAll', { count }))
  }
  ...
  try {
    ...
    if (data.success) {
      showToast(data.message || t('apiKeys.allDeletedCleared'), 'success')

      // 如果有失败的，显示详细信息
      if (data.details && data.details.failedCount > 0) {
        const errors = data.details.errors
        console.error('Some API Keys clear failed:', errors)
        showToast(`${data.details.failedCount} ${t('apiKeys.clearFailed')}`, 'warning')
      }

      // 刷新已删除列表
      loadDeletedApiKeys()
    } else {
      showToast(data.error || t('apiKeys.clearFailed'), 'error')
    }
  } catch (error) {
    showToast(error.response?.data?.error || t('apiKeys.clearFailed'), 'error')
  }
}
...
const batchDeleteApiKeys = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (successCount > 0) {
        showToast(t('apiKeys.batchSuccess', { count: successCount }), 'success')
    ...
    showToast(t('apiKeys.operationFailed'), 'error')
    console.error('Batch delete API Keys failed:', error)
  }
}
...
const copyApiStatsLink = (apiKey) => {
  ...
  try {
    ...
    if (successful) {
      showToast(t('apiKeys.linkCopied'), 'success')
    } else {
      showToast(t('apiKeys.copyFailed'), 'error')
      console.log('Statistics page link:', statsUrl)
    }
  } catch (err) {
    showToast(t('apiKeys.copyFailed'), 'error')
...
const handleSaveExpiry = async ({ keyId, expiresAt, activateNow }) => {
  try {
    ...
    if (data.success) {
      showToast(activateNow ? t('apiKeys.keyActivated') : t('apiKeys.expiryUpdated'), 'success')
      // 更新本地数据
      ...
      showToast(data.message || t('apiKeys.updateFailed'), 'error')
      // 重置保存状态
    ...
    showToast(t('apiKeys.updateFailed'), 'error')
    // 重置保存状态
    if (expiryEditModalRef.value) {
      expiryEditModalRef.value.resetSaving()
    }
...
const formatLastUsed = (dateString) => {
  ...
  if (diff < 3600000) return t('apiKeys.minutesAgo', { minutes: Math.floor(diff / 60000) })
  if (diff < 86400000) return t('apiKeys.hoursAgo', { hours: Math.floor(diff / 3600000) })
  if (diff < 604800000) return t('apiKeys.daysAgo', { days: Math.floor(diff / 86400000) })
